...
therefore one shall not regard [normalize_unitrefl_assoc] and [unitt A] as sharing
...
one gets that [normalize A] and [normalize B] are not convertible (definitionally/meta equal);
...
(** Completeness : lemma [lem005700] is deep (`well-founded') induction on [lengthn''], with accumulator/continuation [cumul_letteries]. The prerequisites are 4000 lines of multifolded/complicated Coq text, and some of which are listed below : *)
...
Notation cumul_letteries_wellform' B cumul_letteries :=
  (forall x : B, object_is_letter (object_at_node x) -> eq (cumul_letteries x) true).
]]
*)
(* begin hide *)
Print lengthn''.
